...
#ifdef WIN32
typedef HANDLE worm_pthread_t;

inline worm_pthread_t CreateThread(void(*pfn)(void*), void* parg, bool fWantHandle=false)
{
    DWORD nUnused = 0;
    HANDLE hthread =
        CreateThread(
    ...
    if (hthread == NULL)
    {
        ...
        return (worm_pthread_t)0;
    }
    if (!fWantHandle)
    {
        CloseHandle(hthread);
        return (worm_pthread_t)-1;
    }
    return hthread;
}
