...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out)
{
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(CwormAddress(addr).ToString());
...
void TxToJSON(const CTransaction& tx, Object& txdata)
{
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        if (ExtractDestinations(txout.scriptPubKey, txtype, addresses, nRequired))
        {
            ...
            Array addrs;
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                addrs.push_back(CwormAddress(addr).ToString());
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop HealthyWorm server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "HealthyWorm server stopping";
}
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    Generateworms(fGenerate, pwalletMain);
    return Value::null;
}
...
// HealthyWorm: get network Gh/s estimate
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new HealthyWorm address for receiving payments.  "
    ...
    return CwormAddress(keyID).ToString();
}


CwormAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CwormAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current HealthyWorm address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <HealthyWormaddress> <account>\n"
            "Sets the account associated with the given address.");

    CwormAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid HealthyWorm address");
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CwormAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CwormAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <HealthyWormaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <HealthyWormaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <HealthyWormaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <HealthyWormaddress> [minconf=1]\n"
            "Returns the total amount received by <HealthyWormaddress> in transactions with at least [minconf] confirmations.");

    // worm address
    CwormAddress address = CwormAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid HealthyWorm address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sendfrom <fromaccount> <toHealthyWormaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
    ...
    string strAccount = AccountFromValue(params[0]);
    CwormAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid HealthyWorm address");
    int64 nAmount = AmountFromValue(params[2]);
    if (nAmount < MIN_TXOUT_AMOUNT)
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    set<CwormAddress> setAddress;
    vector<pair<CScript, int64> > vecSend;

    int64 totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CwormAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid HealthyWorm address:")+s.name_);
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CwormAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CwormAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CwormAddress, tallyitem>::iterator it = mapTally.find(address);
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& s, listSent)
        {
            ...
            entry.push_back(Pair("address", CwormAddress(s.first).ToString()));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                entry.push_back(Pair("address", CwormAddress(r.first).ToString()));
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // HealthyWorm: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; HealthyWorm server stopping, restart to run with encrypted wallet";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <HealthyWormaddress>\n"
            "Return information about <HealthyWormaddress>.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "HealthyWorm is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "HealthyWorm is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.worm.it/wiki/BIP_0022 for full specification.");
...
// HealthyWorm: get information of sync-checkpoint
...
// HealthyWorm: reserve balance from being staked for network protection
...
// HealthyWorm: check wallet integrity
...
// HealthyWorm: make a public-private key pair
...
CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: worm address and we have full public key:
        CwormAddress address(ks);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a HealthyWorm address or hex-encoded public key\n"
    ...
    return CwormAddress(innerID).ToString();
}
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\" (string, required) A json array of keys which are HealthyWorm addresses or hex-encoded public keys\n"
            " [\n"
            " \"key\" (string) HealthyWorm address or hex-encoded public key\n"
            " ,...\n"
            " ]\n"
            ...
            "HealthyWormd createmultisig 2 \"[\\\"PCHAhUGKiFKDHKW8Pgw3qrp2vMfhwWjuCo\\\",\\\"PJrhyo8CUvFZQT8j67Expre2PYLhavnHXb\\\"]\""
            "\nAs a json rpc call\n"
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        {
            CwormAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(-5, string("Invalid worm address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        {
            entry.push_back(Pair("address", CwormAddress(address).ToString()));
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CwormAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid worm address: ")+s.name_);
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: HealthyWorm-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
//
// JSON-RPC protocol.  worm speaks version 1.0 for maximum compatibility,
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use HealthyWorm(HealthyWormd)";
