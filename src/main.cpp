...
const string strMessageMagic = "HealthyWorm Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // HealthyWorm: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // HealthyWorm: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // HealthyWorm: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// HealthyWorm: find block wanted by given orphan block
...
// HealthyWorm: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge, int nHeight)
{
	int64 nRegularRate = 17;
	int64 nSubsidy = 2017 * nCoinAge * 33 / (365 * 33 + 8) * CENT;
	if (nHeight > 150000) { nSubsidy = nRegularRate * nCoinAge * 33 / (365 * 33 + 8) * CENT; }
    return nSubsidy;
}
...
// HealthyWorm: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // HealthyWorm: target change every block
    // HealthyWorm: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // HealthyWorm: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal worm miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // HealthyWorm: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // HealthyWorm: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // HealthyWorm: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // HealthyWorm: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // HealthyWorm: track money supply and mint amount info
    ...
    // HealthyWorm: fees are not collected by miners as in worm
    // HealthyWorm: fees are destroyed to compensate the entire network
...
// HealthyWorm: total coin age spent in transaction, in the unit of coin-days.
...
// HealthyWorm: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // HealthyWorm: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // HealthyWorm: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // HealthyWorm: record proof-of-stake hash value
    ...
    // HealthyWorm: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // HealthyWorm: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // HealthyWorm: coinbase output should be empty if proof-of-stake block
    ...
    // HealthyWorm: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // HealthyWorm: check that the block satisfies synchronized checkpoint
    ...
    // HealthyWorm: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // HealthyWorm: check proof-of-stake
    ...
    // HealthyWorm: verify hash target and signature of coinstake tx
    ...
    // HealthyWorm: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // HealthyWorm: getblocks may not obtain the ancestor block rejected
    ...
    // HealthyWorm: if responsible for sync-checkpoint send it
...
// HealthyWorm: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "HealthyWorm", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "HealthyWorm", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // HealthyWorm: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // HealthyWorm: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // HealthyWorm: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // HealthyWorm: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // HealthyWorm: if detected invalid checkpoint enter safe mode
    ...
    // HealthyWorm: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // HealthyWorm: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // HealthyWorm: record my external IP reported by peer
        ...
        // HealthyWorm: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // HealthyWorm: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // HealthyWorm: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // HealthyWorm: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // HealthyWorm: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("wormMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadwormMiner(void* parg);

static bool fGenerateworms = false;
...
void wormMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateworms || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateworms) && !fProofOfStake)
                return;
        }
        ...
        if (fProofOfStake)
        {
            // HealthyWorm: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
...
void static ThreadwormMiner(void* parg)
{
    ...
    try
    {
        ...
        wormMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadwormMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadwormMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadwormMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generateworms(bool fGenerate, CWallet* pwallet)
{
    fGenerateworms = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateworms = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d wormMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadwormMiner, pwallet))
                printf("Error: CreateThread(ThreadwormMiner) failed\n");
            Sleep(10);
        }
    }
}
