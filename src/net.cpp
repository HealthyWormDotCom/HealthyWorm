...
// Copyright (c) 2009-2012 The worm Core developers
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "HealthyWorm " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are HealthyWorm dns seeds.
static const char *strDNSSeed[][2] = {
    {"98.144.161.18", "98.144.161.18"},
    {"164.132.189.192", "164.132.189.192"},
    {"seednode", "worm.healthyworm.com"},
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // HealthyWorm enables dns seeding with testnet too
    {
...
// HealthyWorm: stake minter thread
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer.  HealthyWorm is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(void* parg)
{
    ...
    Generateworms(GetBoolArg("-gen", false), pwalletMain);

    // HealthyWorm: mint proof-of-stake blocks in the background
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_MINER] > 0) printf("ThreadwormMiner still running\n");
