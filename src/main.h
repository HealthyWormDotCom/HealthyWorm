...
static const int64 nMaxClockDrift = 60 * 60 * 2;
...
public:
    ...
    {
        // HealthyWorm: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    /** Amount of worms spent by this transaction.
    ...
    /** Amount of worms coming in to this transaction
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // HealthyWorm: get transaction coin age
...
public:
    ...
    // HealthyWorm: block signature - signed by coin base txout[0]'s owner
    ...
    // HealthyWorm: two types of block: proof-of-work or proof-of-stake
    ...
    // HealthyWorm: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // HealthyWorm: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // HealthyWorm: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // HealthyWorm: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // HealthyWorm: block index flags
    enum  
    {
    ...
    /**
     * Duplicate from wormrpc that originaly define this method.
