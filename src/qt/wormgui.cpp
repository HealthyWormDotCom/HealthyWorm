 ...
 * The HealthyWorm Developers 2016-2017
 */
#include "wormgui.h"
...
{
    resize(750, 550);
    setWindowTitle(tr("HealthyWorm Wallet"));
#ifndef Q_WS_MAC
    setWindowIcon(QIcon(":icons/HealthyWorm_icon"));
#else
...
void wormGUI::createActions()
{
	...
	mintingAction = new QAction(tr("&Mining"), this);
	...
	mintingAction->setProperty("objectName","mintingAction");
	...
	sendCoinsAction->setToolTip(tr("Send coins to a HealthyWorm address"));
    sendCoinsAction->setCheckable(true);
    sendCoinsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_2));
	sendCoinsAction->setProperty("objectName","sendCoinsAction"); 
    tabGroup->addAction(sendCoinsAction);
	
	onthewebAction = new QAction(tr("&Browser"), this);
    onthewebAction->setToolTip(tr("Visit links related to HealthyWorm"));
    onthewebAction->setCheckable(true);
    onthewebAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_7));
	onthewebAction->setProperty("objectName","onthewebAction");
    ...
    aboutAction = new QAction(QIcon(":/icons/HealthyWorm_tooltip"), tr("&About HealthyWorm"), this);
    aboutAction->setToolTip(tr("Show information about HealthyWorm"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for HealthyWorm"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/HealthyWorm_tooltip"), tr("Show/Hide &HealthyWorm"), this);
    toggleHideAction->setToolTip(tr("Show or hide the HealthyWorm window"));
...
void wormGUI::_addButtonInToolbar(QAction *action,QToolBar *toolbar)
{
...
void wormGUI::createToolBars()
{
 ...
 _addButtonInToolbar(mintingAction,toolbar);
 _addButtonInToolbar(onthewebAction,toolbar);
#ifdef FIRST_CLASS_MESSAGING
 _addButtonInToolbar(messageAction,toolbar);
#endif

 addToolBar(Qt::LeftToolBarArea,toolbar);
}

void wormGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        if(clientModel->isTestNet())
        {
            ...
            setWindowIcon(QIcon(":icons/HealthyWorm_icon"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/HealthyWorm_icon"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(title_testnet);
                trayIcon->setIcon(QIcon(":/icons/HealthyWorm_tooltip"));
            }
        }
...
void wormGUI::setWalletModel(WalletModel *walletModel)
{
...
void wormGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("HealthyWorm client"));
    trayIcon->setIcon(QIcon(":/icons/HealthyWorm_tooltip"));
...
#ifndef Q_WS_MAC
void wormGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
    if(reason == QSystemTrayIcon::Trigger)
    {
        // Click on system tray icon triggers "show/hide worm"
        toggleHideAction->trigger();
    }
}
#endif

void wormGUI::toggleHidden()
{
...
void wormGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to HealthyWorm network", "", count));
}

void wormGUI::setNumBlocks(int count)
{
    ...
    if(count < nTotalBlocks)
    {
        ...
        {
            progressBarLabel->setText(tr("<b><FONT COLOR='#ff0000'>** NOT SYNCED **</font></b>"));
            progressBarLabel->setVisible(true);
            progressBar->setFormat(tr("  Local copy of WORM blockchain is %n% downloaded  ", "", nPercentageDone));
...
void wormGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void wormGUI::changeEvent(QEvent *e)
{
...
void wormGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_WS_MAC // Ignored on Mac
...
void wormGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit. You can still send it for a fee of %1. Do you want to pay the fee? It will be destroyed and not paid to anyone").arg(
                wormUnits::formatWithUnit(wormUnits::BTC, nFeeRequired));
...
void wormGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
        ...
        notificator->notify(Notificator::Information,
                            (amount)<0 ? tr("Sent transaction") :
                              ...
                              tr("Date: %1\n"
                                 ...
                                 "Address: %4\n")
                              .arg(date)
                              .arg(wormUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                              .arg(type)
                              .arg(address), icon);
    }
}

void wormGUI::gotoOverviewPage()
{
...
void wormGUI::gotoAddressBookPage()
{
...
void wormGUI::gotoReceiveCoinsPage()
{
...
void wormGUI::gotoSendCoinsPage()
{
...
void wormGUI::gotoMultisigPage()
{
    multisigPage->show();
    multisigPage->setFocus();
}

 void wormGUI::gotoOnthewebPage()
 {
...
void wormGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void wormGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        gotoSendCoinsPage();
...
void wormGUI::handleURI(QString strURI)
{
    gotoSendCoinsPage();
...
void wormGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void wormGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void wormGUI::unlockForMinting(bool status)
{
    if(!walletModel)
        return;
...
void wormGUI::showNormalIfMinimized()
{
...
void wormGUI::applyTheme(QString name)
{
...
// file.setFileName("themes"+QString(QDir::separator())+ name + QString(QDir::separator())+ name + ".qss");
 ...
 setMinimumSize(1100,700);  // You can remove this line just give good idea of theme at this resolution - Yash
}