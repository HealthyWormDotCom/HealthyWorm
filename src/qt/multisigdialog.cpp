...
void MultisigDialog::on_saveMultisigAddressButton_clicked()
{
    ...
    if(!wallet->HaveCScript(scriptID))
        wallet->AddCScript(script);
    if(!wallet->mapAddressBook.count(CwormAddress(address).Get()))
        wallet->SetAddressBookName(CwormAddress(address).Get(), label);
}
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                CwormAddress address(recipient.address.toStdString());
