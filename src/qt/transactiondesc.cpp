...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
...

    {
                    ...
                    if (wallet->IsMine(txout))
                    {
                        ...
                        if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address))
                        {
                            if (wallet->mapAddressBook.count(address))
                            {
                                ...
                                strHTML += tr("<b>To:</b> ");
                                strHTML += GUIUtil::HtmlEscape(CwormAddress(address).ToString());
        ...
        if (!wtx.mapValue["to"].empty())
        {
            ...
            strHTML += tr("<b>To:</b> ");
            CTxDestination dest = CwormAddress(strAddress).Get();
        ...
        if (wtx.IsCoinBase() && nCredit == 0)
        {
            ...
            if (wtx.IsInMainChain())
                strHTML += tr("(%1 matures in %2 more blocks)")
                        .arg(wormUnits::formatWithUnit(wormUnits::BTC, nUnmatured))
        ...
        else if (nNet > 0)
        {
            ...
            // Credit
            //
            strHTML += tr("<b>Credit:</b> ") + wormUnits::formatWithUnit(wormUnits::BTC, nNet) + "<br>";
        }
        else
        {
            ...
            if (fAllFromMe)
            {
                ...
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + wormUnits::formatWithUnit(wormUnits::BTC, -txout.nValue) + "<br>";
                }
                ...
                if (fAllToMe)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + wormUnits::formatWithUnit(wormUnits::BTC, -nValue) + "<br>";
                    strHTML += tr("<b>Credit:</b> ") + wormUnits::formatWithUnit(wormUnits::BTC, nValue) + "<br>";
                }

                int64 nTxFee = nDebit - wtx.GetValueOut();
                if (nTxFee > 0)
                    strHTML += tr("<b>Transaction fee:</b> ") + wormUnits::formatWithUnit(wormUnits::BTC,-nTxFee) + "<br>";
            }
            else
            {
                //
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                    if (wallet->IsMine(txin))
                        strHTML += tr("<b>Debit:</b> ") + wormUnits::formatWithUnit(wormUnits::BTC,-wallet->GetDebit(txin)) + "<br>";
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                    if (wallet->IsMine(txout))
                        strHTML += tr("<b>Credit:</b> ") + wormUnits::formatWithUnit(wormUnits::BTC,wallet->GetCredit(txout)) + "<br>";
            }
        }

        if((!wtx.IsCoinStake() && !wtx.IsCoinBase()) || wtx.GetBlocksToMaturity() == 0)
        {
            strHTML += tr("<b>Net amount:</b> ") + wormUnits::formatWithUnit(wormUnits::BTC,nNet, true) + "<br>";
        }else{
            strHTML += tr("<b>Retained amount:</b> %1 until %2 more blocks<br>").arg(wormUnits::formatWithUnit(wormUnits::BTC,-nNet)).arg(wtx.GetBlocksToMaturity());
        }
        ...
        if (fDebug)
        {
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                if(wallet->IsMine(txin))
                    strHTML += "<b>Debit:</b> " + wormUnits::formatWithUnit(wormUnits::BTC,-wallet->GetDebit(txin)) + "<br>";
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if(wallet->IsMine(txout))
                    strHTML += "<b>Credit:</b> " + wormUnits::formatWithUnit(wormUnits::BTC,wallet->GetCredit(txout)) + "<br>";
                    ...
                    if(txdb.ReadDiskTx(prevout.hash, prev))
                    {
                        if (prevout.n < prev.vout.size())
                        {
                            ...
                            if (ExtractDestination(vout.scriptPubKey, address))
                            {
                                if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].empty())
                                    strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address]) + " ";
                                strHTML += QString::fromStdString(CwormAddress(address).ToString());
                            }
                            strHTML = strHTML + " Amount=" + wormUnits::formatWithUnit(wormUnits::BTC,vout.nValue);
